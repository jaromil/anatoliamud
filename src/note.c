...
void save_notes(int type)
{
	...
	default:
	    return;
	case NOTE_NOTE:
	    name = ana_config.note_note_file;
	    pnote = note_list;
	    break;
	case NOTE_IDEA:
	    name = ana_config.note_idea_file;
	    pnote = idea_list;
	    break;
	case NOTE_PENALTY:
	    name = ana_config.note_penalty_file;
	    pnote = penalty_list;
	    break;
	case NOTE_NEWS:
	    name = ana_config.note_news_file;
	    pnote = news_list;
	    break;
	case NOTE_CHANGES:
	    name = ana_config.note_changes_file;
	    pnote = changes_list;
	    break;
    }
...
{
    load_thread(ana_config.note_note_file,&note_list, NOTE_NOTE, 14*24*60*60);
    load_thread(ana_config.note_idea_file,&idea_list, NOTE_IDEA, 28*24*60*60);
    load_thread(ana_config.note_penalty_file,&penalty_list, NOTE_PENALTY, 0);
    load_thread(ana_config.note_news_file,&news_list, NOTE_NEWS, 0);
    load_thread(ana_config.note_changes_file,&changes_list,NOTE_CHANGES, 0);
}
...
void load_thread(char *name, NOTE_DATA **list, int type, time_t free_time)
{
    ...
    strcpy( strArea, ana_config.note_note_file );
    fpArea = fp;
